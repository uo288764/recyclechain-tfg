...
dev:
	...
	@until docker compose -f $(BACKEND_DIR)/docker-compose.yml exec -T $(DB_SERVICE) pg_isready -U $(DB_USER) > /dev/null 2>&1; do \
		printf "."; \
		sleep 1; \
	done
	@echo ""
	...
	@until echo "$$(curl -s -o /dev/null -w '%{http_code}' http://localhost:$(BACKEND_PORT)/api/auth/login)" | grep -qE "^(403|405)$$"; do \
		printf "."; \
		sleep 2; \
	done
	@echo ""
